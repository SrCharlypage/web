...
try {
    ...
    $nextNumero = 'Error al calcular el número';
}
?>
...
<head>
    ...
    <style>
        ...
        .page-footer .footer-year {
            color: #000; /* y otro distinto para el año */
        }
    </style>
</head>
<body>
    <main class="form-wrapper">
        ...
        <form action="registra_hermano.php" method="POST" enctype="multipart/form-data">
            <!-- Nº Hermano -->
            ...
            <div class="row-flex">
                <div class="form-group">
                    ...
                    <input type="text" id="direccion" name="direccion" required placeholder="Ej. Avd./Calle/Plaza">
                </div>
            </div>

            <div class="row-flex">
                <div class="form-group">
                    <label for="poblacion"><i class="fa fa-home"></i> Población</label>
                    <input type="text" id="poblacion" name="poblacion" required placeholder="Ej. Población">
                </div>
                <div class="form-group">
                    <label for="provincia"><i class="fa fa-home"></i> Provincia</label>
                    <input type="text" id="provincia" name="provincia" required placeholder="Ej. Provincia">
                </div>
                <div class="form-group">
                    <label for="cod_postal"><i class="fa fa-home"></i> Cód. Postal</label>
                    <input type="text" id="cod_postal" name="cod_postal" required placeholder="Ej. 11100">
                </div>
            </div>
            <!-- DNI, Fecha y Sexo -->
            <div class="row-flex">
		...
		document.querySelector('form').addEventListener('submit', function(event) {
			...
			requiredFields.forEach(field => {
				if (!field.value.trim()) {
					...
					errorMessages.push(`El campo ${field.id} es obligatorio.`);
				} else {
			...
			if (dniField.value && !dniRegex.test(dniField.value)) {
				isValid = false;
				dniField.style.borderColor = '#dc3545';
				errorMessages.push('El formato del DNI no es válido. Debe ser 8 dígitos seguidos de una letra.');
			}

			// Validar formato de código postal
			const codPostalField = document.getElementById('cod_postal');
			const codPostalRegex = /^\d{5}$/;
			if (codPostalField.value && !codPostalRegex.test(codPostalField.value)) {
				isValid = false;
				codPostalField.style.borderColor = '#dc3545';
				errorMessages.push('El código postal debe contener solo 5 dígitos.');
			}
